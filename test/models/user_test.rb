...
class UserTest < ActiveSupport::TestCase
  def setup
    @user = User.new(name: "TestUser", email: "test@example.com", password: "password123", password_confirmation: "password123")
  end
  ...
  test "email validation should not accept invalid email addresses" do
    valid_addresses = %w[tony@example,com test_at_example.com test@example. test@ex+ample.com]
    valid_addresses.each do |valid_address|
      @user.email = valid_address
  ...
  test "should follow and unfollow a user" do
    ...
    tony = users(:tony)
    assert_not michael.following?(tony)
    michael.follow(tony)
    assert michael.following?(tony)
    assert tony.followers.include?(michael)
    michael.unfollow(tony)
    assert_not michael.following?(tony)
  end
end
